...
import { Users, MapPin, Clock, History, UserCheck, Trash2, Printer, ChevronDown } from 'lucide-react';
import { Button } from '@/components/ui/button';

export default function SAManagement() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-16">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">S.A. Management</h1>
        <p className="text-sm text-gray-500 mt-2 font-medium">Assign Student Assistants to labs, monitor duty, and view reports.</p>
      </div>

      {/* Lab Overview — Who's on duty */}
      ...
      {/* Teacher Lab Reports with SA on Duty */}
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        <button onClick={() => setShowReports(!showReports)} className="w-full p-6 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-bold text-gray-400 bg-gray-50 px-2 py-1 rounded-full">{teacherLabReports.length} reports</span>
          </div>
        ...
        {showReports && (
          <div className="p-6 pt-0">
            <div className="flex justify-end mb-4">
              <Button variant="outline" size="sm" className="rounded-xl text-xs font-bold gap-1.5" onClick={() => {
                const w = window.open('', '_blank');
                if (!w) return;
                w.document.write(`<html><head><title>SA Duty Reports</title><style>body{font-family:'Segoe UI',sans-serif;padding:40px;color:#111}h1{font-size:20px}table{width:100%;border-collapse:collapse;font-size:12px;margin-top:20px}th{background:#f3f4f6;text-align:left;padding:10px;font-size:10px;text-transform:uppercase;border-bottom:2px solid #e5e7eb}td{padding:8px 10px;border-bottom:1px solid #f3f4f6}</style></head><body><h1>Teacher Lab Reports — SA on Duty</h1><table><thead><tr><th>Teacher</th><th>Lab</th><th>Title</th><th>SA on Duty</th><th>Submitted</th></tr></thead><tbody>${teacherLabReports.map((r) => `<tr><td>${r.teacherName}</td><td>${r.labName}</td><td>${r.title}</td><td>${r.saName || 'None'}</td><td>${new Date(r.submittedAt).toLocaleString()}</td></tr>`).join('')}</tbody></table></body></html>`);
                w.document.close();
                w.print();
              }}>
            ...
            {teacherLabReports.length === 0 ? (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[9px] font-bold tracking-widest text-gray-400 uppercase border-b border-gray-100">
                    <th className="pb-3">Teacher</th><th className="pb-3">Lab</th><th className="pb-3">Report Title</th><th className="pb-3">SA on Duty</th><th className="pb-3">Submitted</th>
                  </tr>
                </thead>
                <tbody className="text-xs font-medium text-gray-700 divide-y divide-gray-50">
                    ...
                    <tr key={r.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="py-3">
                        {r.saName ? (
                          ...
                          <span className="text-gray-400 italic">No SA assigned</span>
                        )}
                      </td>
