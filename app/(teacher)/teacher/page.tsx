...
import {
  ...
  ClipboardList, Bell, Plus, Send, Lock
} from 'lucide-react';
...
export default function TeacherDashboard() {
  ...
  const handleNewReport = () => {
    if (!canCreateReport) { toast.error('Unauthorized action.'); return; }
    if (containsScriptInjection(reportTitle) || containsScriptInjection(reportDesc)) { toast.error('Invalid input detected.'); return; }
    const title = sanitizeInput(reportTitle);
    const desc = sanitizeInput(reportDesc);
    if (!title || title.length < 3) { toast.error('Title must be at least 3 characters.'); return; }
    createTicket({ userId, type: 'incident', title, description: desc || 'No description provided.', status: 'open', priority: 'medium', lab: 'Lab 1' });
    createReport({ teacherId: userId, title, description: desc });
    logAudit('REPORT_SUBMITTED', userId, currentUser?.name || '', role || '', `Submitted report: ${title}`);
  ...
  const handleNewSoftwareRequest = () => {
    if (!canCreateSoftware) { toast.error('Unauthorized action.'); return; }
    if (containsScriptInjection(swName)) { toast.error('Invalid input detected.'); return; }
    const name = sanitizeInput(swName);
    const lab = sanitizeInput(swLab);
    if (!name || !lab) { toast.error('Please fill in all fields.'); return; }
    createSoftwareRequest({ teacherId: userId, softwareName: name, lab, status: 'pending' });
    logAudit('SOFTWARE_REQUESTED', userId, currentUser?.name || '', role || '', `Requested: ${name} for ${lab}`);
    toast.success('Software request submitted!');
    setSwName(''); setSwLab(''); setShowSoftwareForm(false);
  };
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-16">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-2 font-medium">Welcome, {userName}! · RBAC enforced</p>
        </div>
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" className="rounded-xl border-gray-200 text-gray-500 h-10 w-10"><Bell className="h-4 w-4" /></Button>
          {canCreateReport ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-[#f8fafc] border border-gray-100 p-6 rounded-3xl">
          <div className="flex justify-between items-start mb-4"><h3 className="text-[11px] font-bold text-gray-400 tracking-wider uppercase">Active Classes</h3><GraduationCap className="h-4 w-4 text-gray-600" /></div>
          <p className="text-3xl font-bold text-gray-900">04</p>
          <p className="text-[10px] font-bold text-[#16a34a] mt-2">2 labs in use</p>
        </div>
        <div className="bg-[#f8fafc] border border-gray-100 p-6 rounded-3xl">
          <div className="flex justify-between items-start mb-4"><h3 className="text-[11px] font-bold text-gray-400 tracking-wider uppercase">Software Reqs</h3><Settings2 className="h-4 w-4 text-gray-600" /></div>
        ...
        <div className="bg-[#f8fafc] border border-gray-100 p-6 rounded-3xl">
          <div className="flex justify-between items-start mb-4"><h3 className="text-[11px] font-bold text-gray-400 tracking-wider uppercase">Lab Utilization</h3><BarChart3 className="h-4 w-4 text-gray-600" /></div>
        ...
        <div className="bg-[#f8fafc] border border-gray-100 p-6 rounded-3xl">
          <div className="flex justify-between items-start mb-4"><h3 className="text-[11px] font-bold text-gray-400 tracking-wider uppercase">Reported Issues</h3><Bug className="h-4 w-4 text-gray-600" /></div>
        ...
        <div className="col-span-1 space-y-8">
          <div className="bg-[#f8fafc] border border-gray-100 rounded-[2rem] p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              {canCreateSoftware ? (
                <Button size="sm" variant="outline" className="rounded-full text-xs" onClick={() => setShowSoftwareForm(!showSoftwareForm)}><Plus className="h-3 w-3" /></Button>
              ) : (
                <Lock className="h-4 w-4 text-gray-300" />
              )}
            </div>
            {showSoftwareForm && canCreateSoftware && (
            ...
            <div className="space-y-4">
              {myRequests.map((r) => (
                <div key={r.id} className="bg-white p-5 rounded-2xl flex items-center justify-between shadow-sm border border-transparent hover:border-gray-200 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="h-10 w-10 bg-blue-50 rounded-xl flex items-center justify-center"><ClipboardList className="h-5 w-5 text-[#164ac9]" /></div>
                    <div>
                  ...
                  {r.status === 'approved' ? <CheckCircle2 className="h-5 w-5 text-emerald-500" /> : <span className="text-[10px] font-bold text-amber-500 uppercase">{r.status}</span>}
                </div>
              ))}
            </div>
          </div>
          <div className="bg-[#f8fafc] border border-gray-100 rounded-[2rem] p-8 space-y-8">
            <div>
              <div className="flex items-center gap-3 mb-5"><div className="bg-[#164ac9] rounded-md p-1"><CheckCircle2 className="h-4 w-4 text-white" /></div><h3 className="font-bold text-sm text-gray-900">Security Status</h3></div>
              <ul className="space-y-3.5 px-1">
                <li className="flex items-center gap-3 text-xs font-semibold text-gray-500"><CheckCircle2 className="h-4 w-4 text-[#164ac9]" /> JWT session active</li>
                <li className="flex items-center gap-3 text-xs font-semibold text-gray-500"><CheckCircle2 className="h-4 w-4 text-[#164ac9]" /> RBAC enforced on mutations</li>
                <li className="flex items-center gap-3 text-xs font-semibold text-gray-500"><CheckCircle2 className="h-4 w-4 text-[#164ac9]" /> Input sanitization active</li>
              </ul>
            </div>
            <div className="pt-2">
              <div className="flex items-center gap-3 mb-5"><Shield className="h-5 w-5 text-gray-600" /><h3 className="font-bold text-sm text-gray-900">Non-Functional Req.</h3></div>
              <ul className="space-y-3.5 px-1">
                <li className="flex items-center gap-3 text-xs font-semibold text-gray-500"><CheckCircle2 className="h-4 w-4 text-gray-400" /> All actions audit-logged</li>
                <li className="flex items-center gap-3 text-xs font-semibold text-gray-500"><CheckCircle2 className="h-4 w-4 text-gray-400" /> Session auto-expiry (30min)</li>
              </ul>
            </div>
          </div>
        </div>
