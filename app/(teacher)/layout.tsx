    ...
    <div className="min-h-screen bg-gray-50/20 flex font-sans">
      <aside className="w-64 bg-[#f8fafc] border-r border-gray-100 flex flex-col hidden lg:flex">
        ...
        <div className="flex-1 py-8 px-4 flex flex-col gap-1">
          <p className="px-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Teacher Menu</p>
          {navigation.map((item) => {
        ...
        <div className="p-4 border-t border-gray-100 mt-auto bg-white">
          <div className="flex items-center gap-3 mb-4 px-2">
            <div className="h-10 w-10 rounded-full bg-amber-100 text-amber-700 font-bold flex items-center justify-center text-sm ring-4 ring-white shadow-sm">
              {initials}
            </div>
            <div className="flex flex-col">
              <span className="text-sm font-bold text-gray-900">{currentUser?.name || 'Teacher'}</span>
              <span className="text-xs text-gray-500 font-medium capitalize">{currentUser?.role || 'Teacher'}</span>
            </div>
          </div>
