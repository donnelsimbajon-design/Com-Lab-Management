    ...
    <div className="min-h-screen bg-gray-50/20 flex font-sans">
      <aside className="w-64 bg-[#f8fafc] border-r border-gray-100 flex flex-col hidden lg:flex">
        <div className="h-16 flex items-center gap-3 px-6 border-b border-gray-100 bg-white">
          <div className="h-8 w-8 bg-[#164ac9] rounded-lg flex items-center justify-center">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-colors ${
                  isActive ? 'bg-blue-50 text-blue-600 font-semibold' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'
                }`}
              >
        ...
        <div className="p-4 border-t border-gray-100 mt-auto bg-white">
          <div className="flex items-center gap-3 mb-4 px-2">
            <div className="h-10 w-10 rounded-full bg-blue-100 text-blue-700 font-bold flex items-center justify-center text-sm ring-4 ring-white shadow-sm">
              {initials}
            </div>
            <div className="flex flex-col">
              <span className="text-sm font-bold text-gray-900">{currentUser?.name || 'Student'}</span>
              <span className="text-xs text-gray-500 font-medium capitalize">{currentUser?.role || 'Student'}</span>
            </div>
          </div>
